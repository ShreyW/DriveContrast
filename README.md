...
The full Waymo Open Dataset alongside the full Impromptu-VLA QA dataset is massive. To accommodate PACE constrained storage limits (e.g., under 300 GB), this project utilizes a subset of the pre-processed trajectory data provided by [Impromptu-VLA](https://github.com/ahydchh/Impromptu-VLA).

To download only a fraction of the data directly from HuggingFace:

1. Run the provided download script (`download_waymo_subset.py`) which selects 2 training shards and 1 validation shard. This provides a mix of examples while keeping the total footprint under ~50-100GB.
```bash
python download_waymo_subset.py
```

2. Point the `--dataset_dir` in the tokenizer/training scripts to the newly downloaded `./data/waymo_subset` directory.
